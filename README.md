...
`/explore/` and `/en/explore/` provide a separate bilingual preview: a personal introduction, a question, three research phases, six interactive report charts, and interest-based links to existing work. The main homepage is unchanged. Method notes are under `/method/{eye-tracking,facial-expression-analysis,eeg,emg,behavioral-tasks}/`, with matching English routes.

The story uses scene-specific actions: a full-bleed portrait-and-cup cover, three distinct evidence-entry choices, a draggable three-phase filmstrip with a tap alternative, observation hotspots, a bright morning task and an explicit report reveal. Generic slide disclosure is retired. Charts draw on reveal and dataset changes, respecting reduced motion. A local Web Audio score attempts autoplay on entry and starts on the first interaction if blocked by the browser. It has an explicit on/off switch and volume control; it pauses in the background and stops on navigation. Run `node scripts/explore-dynamic-audit.mjs` for disclosure, chart animation and audio checks.
