...
## Homepage and interactive story

The eight-scene story includes evidence-entry choices, observation-to-record explanations, guided report charts, measurement animations, and a delivery synthesis.

`/` and `/en/` open with a personal introduction and two choices: explore a real research case, or go straight to the work overview. The previous homepages remain at `/overview/` and `/en/overview/`. `/explore/` and `/en/explore/` retain direct access to the case: a personal introduction, a question, three research phases, six interactive report charts, and interest-based links to existing work. Existing work and note URLs remain unchanged. Method notes are under `/method/{eye-tracking,facial-expression-analysis,eeg,emg,behavioral-tasks}/`, with matching English routes.

The story uses scene-specific actions: a full-bleed portrait-and-cup cover, three distinct evidence-entry choices, a draggable three-phase filmstrip with a tap alternative, observation hotspots, a bright morning task and an explicit report reveal. Generic slide disclosure is retired. Charts draw on reveal and dataset changes, respecting reduced motion. A local Web Audio score starts when the visitor chooses to enter the case; arriving on the homepage is silent. It has an explicit on/off switch and volume control; it pauses in the background and stops on navigation. Run `node scripts/explore-dynamic-audit.mjs` for disclosure, chart animation and audio checks.
